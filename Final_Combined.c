...
/* ------------------------
   Simple non-blocking web server handler
   - Uses ESP_CheckForClient/ESP_SendWebPage/ESP_CloseConnection
   - Toggle via /toggle
   ------------------------ */
static void HandleWebServer(void)
{
    if (ESP_CheckForClient(req, sizeof(req)) != ESP8266_OK)
        return;  // no client, non-blocking return

    /* If client requested /toggle, flip unit */
    if (strstr(req, "GET /toggle"))
        showFahrenheit ^= 1;

    float t = showFahrenheit ? (currentTemp * 9.0f/5.0f + 32.0f) : currentTemp;

    /* Build a compact HTML page (auto-refresh 2s) */
    snprintf(html, sizeof(html),
        ...
        "<style>"
        "body{font-family:Arial,Helvetica,sans-serif;text-align:center;padding:20px;background:#f7f7f7;color:#111}"
        ".box{background:#fff;padding:20px;border-radius:12px;display:inline-block;box-shadow:0 6px 18px rgba(0,0,0,0.08)}"
        ".temp{font-size:64px;margin:8px 0;color:#e64a19}"
        "button{padding:10px 16px;border-radius:8px;border:none;background:#1976d2;color:#fff;font-size:16px;cursor:pointer}"
        "</style></head><body>"
        "<div class='box'>"
        "<h2>Temperature</h2>"
        "<div class='temp'>%d%c</div>"
        "<a href='/toggle'><button>Toggle</button></a>"
        "<div style='margin-top:8px;font-size:12px;color:#666'>Auto refresh: 2s %s</div>"
        "</div></body></html>",
        (int)t, (showFahrenheit ? 'F' : 'C'),
        (ip_buf[0] != '\0') ? ip_buf : ""
    );
...
int main(void)
{
    ...
    /* --- Initialize ESP8266 and WiFi (non-blocking usage thereafter) --- */
    // NOTE: replace these with your network credentials
    SSD1306_Fill(0);
    ...
    while (1)
    {
        ...
        // Handle web server requests if needed (non-blocking)
        HandleWebServer();
    }
}
